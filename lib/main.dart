...
import 'package:reaprime/src/services/bridge/bridge_discovery_service.dart';
...
void main() async {
  ...
  // Load settings early to get bridge host config
  final settingsService = SettingsService();
  final bridgeHost = await settingsService.bridgeHost();
  final bridgeHttpPort = await settingsService.bridgeHttpPort();
  final bridgeWsPort = await settingsService.bridgeWsPort();

  // Use mDNS auto-discovery unless a manual host is configured
  final bridgeDiscovery = BridgeDiscoveryService(
    manualHost: bridgeHost.isNotEmpty ? bridgeHost : null,
    httpPort: bridgeHttpPort,
    wsPort: bridgeWsPort,
  );
  services.add(bridgeDiscovery);
  ...
  // Forward bridge URL to plugins so fetch() calls to localhost:8080 get redirected
  bridgeDiscovery.devices.listen((devices) {
    final client = bridgeDiscovery.client;
    if (client != null) {
      pluginService.pluginManager.bridgeHttpBaseUrl = client.httpBaseUrl;
    }
  });
